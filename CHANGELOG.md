...
换票先读缓存、同码共用在途请求. 失败留上一笔, 只有点刷新才盖红字.

### 修复：分时切多了画成空白

换票时上一只的分钟点不再套到新轴上. 对不上的轴不下空白线, 空包不进缓存.

### 改进：日经 / 韩国分时从 8 点起轴

主图不再套 A 股 09:30. 东财北京时日经 08:00-10:30+11:30-14:30, KOSPI 08:00-14:30.

### 改进：资讯页热榜 tab 换到卡内换行

腾讯网-综合早报 / 哔哩哔哩不再挤在卡头里, 不用放大也能点.

### 改进：资讯页 NewsNow 和 REBANG 合成一格

微博 / 知乎 / 金十同名去重, NewsNow 优先, REBANG 只补独有 (东方财富). 雪球两边都不拉. 下行变成 X起爆 | 热榜 | AIHOT. 钥匙还是 `event_rank`.

### 移除：全站实时新闻弹窗

不再挂 `NewsToastHost`。快讯只在资讯页 `/event` 看，不另开轮询。

### 改进：资讯页日历和热榜跟着分色

热榜三格各一色, tab 带条数, 日历天数写 N条. 新闻三栏分色还在. 不改钥匙、不另开轮询.

### 改进：资讯页实时新闻栏头分色

财联社 / 新浪见闻 / 金十栏头加色条和条数, 不再是一排灰字. 仍走同一份电报中心.

### 改进：顶栏「事件」改成「资讯」

`/event` 人看见的名字改成资讯。路由和钥匙不动。

### 改进：事件页实时新闻三栏并列

`/event` 实时新闻格内财联社 / 新浪见闻 / 金十各占三分之一，仍走同一份电报中心，钟只续拉已点亮的源。不另开轮询。

### 新增：事件页接入四份热榜，AI 观察挂同一份 AIHOT

`/event` 下行挂 SoPilot X起爆 / NewsNow / REBANG / AIHOT。`GET /api/event/ranks`，钥匙 `event_rank`，180s 上一笔。AIHOT 直拉官方 `/api/v1/items` + `hot-topics` + `dailies/latest`，主题卡解析 `/topics`（官方 v1 无 topics 口）。`/ai-watch` 底栏同一把 `event_rank/aihot`（`?part=aihot`），不另开钥匙。不经 NewsNow。不进复盘预热、不另开快讯轮询。

### 改进：分时/日K 右轴刻度疏一些

nicePriceTicks 默认约 4-5 档, 步长按 1/2/5 往大取. 涨跌幅轴不再 0.5% 一格挤满. 不另开图.

### 修复：恒生 / 恒科 15 点后分时还在画

港股收到 16:00. A 股 15:00 last-good 不再冻 hkHSI / hkHSTECH. 大图轴铺到 16:00 (含 11:30-12:00). 外盘分时 TTL 4s, 叠报价也认外盘. 不另开轮询.

### 改进：快讯 10 秒过期重取, 不是推送

网页 telegraphHub 从 30 秒改 10 秒. 财联社不再 `_dc` 冻到预热钟, 过期跟直播一样重取. 不另开轮询.

### 修复：换票后分时水印还停在上一只

水印名只认当前码. 上一只的 last-good 名字不再留下. 换字先 applyOptions 再重挂.

### 修复：分时水印重新显示股票名

价轴 applyOptions 会把 pane 水印卸掉, applyOptions 还当它在. 改成轴样式之后卸掉再挂. 不另开图.
