...
| 🌊&nbsp;**Options/Futures** | Header `/derivatives` (next to A-share; old `/ovlab` bookmarks redirect): one-screen cockpit — market watch, near-expiry calendar, term structure + warehouse receipts, flow alerts, T-quote (LC IV smile + ATM term on the left, same tquote, OpenVlab hover cards) with option/underlying charts. OpenVlab public REST; MQTT overlays REST in memory (`VR_OVLAB_MQTT=0` off). CTP stays on Portfolio |
