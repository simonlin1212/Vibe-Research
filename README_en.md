...
| 🤖&nbsp;**AI Watch** | OpenRouter public-cloud token share · TrakToken LLM price trend / cut events · AA model table + intelligence×cost scatter (optional key) · AI infra CapEx/ROI (SEC + labeled forecast) · [AIHOT](https://aihot.virxact.com/all) feed + [topics](https://aihot.virxact.com/topics) (same `event_rank/aihot` key as `/event`) |
...
| 🗞️&nbsp;**News** | Header `/event` (next to Arb): live news in three equal lanes (CLS / Sina / Jin10, same telegraph hub) + economic calendar (same Duanxianxia timeline as Jiuyan) + hot ranks (SoPilot / NewsNow+REBANG merged, 微博/知乎/金十 deduped / [AIHOT](https://aihot.virxact.com/all)) |
| ⚡&nbsp;**Duanxianxia** | Header `/dxx` (next to News): auction seals, limit-up tape, upstream emotion fields (not our scores), plate strength, review/mining hit counts. Public no-login feeds only. Click a code for A-share charts |
